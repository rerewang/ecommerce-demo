...
## 报告规范 (Report Standards)

每次完成任务后，**必须**使用中文回复“改动内容摘要”和“如何验收”两部分内容。

### 1. 改动内容摘要
简要列出主要修改的文件、组件或逻辑。
示例：
- **UI 组件新增**: `src/components/ui/NewComponent.tsx`
- **逻辑优化**: 修复了 `ProductService` 中的价格计算 Bug。

### 2. 如何验收 (Acceptance Criteria)
提供详细的分步骤验收指南，确保用户可以复现和验证功能。
示例：
1. 打开首页。
2. 点击“登录”按钮。
3. 输入测试账号。
4. **预期结果**: 跳转至仪表盘，右上角显示用户名。
